...
class SlamMapTab:
    def __init__(self, parent, shared):
        ...
        self.calib_camera_points = []  # [(u,v), ...]
        self.calib_map_points = []     # [(x,y), ...]
        self.homography = None         # 3x3 pixel -> map meter
    ...
    def _build_ui(self):
        ...
        title = ctk.CTkLabel(self.root, text="BAN DO SLAM / RVIZ VIEW", font=("Consolas", 20, "bold"), text_color=C_ACCENT)
        ...
        ctk.CTkButton(top, text="CHON map.yaml", width=130, command=self.load_yaml_dialog).pack(side="left", padx=8, pady=8)
        ctk.CTkButton(top, text="ZOOM +", width=80, command=lambda: self.set_zoom(self.zoom * 1.2)).pack(side="left", padx=4)
        ctk.CTkButton(top, text="ZOOM -", width=80, command=lambda: self.set_zoom(self.zoom / 1.2)).pack(side="left", padx=4)
        ctk.CTkButton(top, text="FIT", width=70, command=self.fit_map).pack(side="left", padx=4)
        ctk.CTkButton(top, text="CLEAR PATH", width=100, fg_color=C_ORANGE, command=self.clear_path).pack(side="left", padx=4)
        ctk.CTkButton(top, text="LOAD H", width=80, command=self.load_homography_dialog).pack(side="left", padx=(14,4))
        ctk.CTkButton(top, text="SAVE H", width=80, command=self.save_homography_dialog).pack(side="left", padx=4)
        ctk.CTkButton(top, text="START ROS LISTEN", width=140, fg_color=C_GREEN, command=self.start_ros_listener).pack(side="left", padx=(18,4))
        ctk.CTkButton(top, text="STOP ROS", width=90, fg_color=C_RED, command=self.stop_ros_listener).pack(side="left", padx=4)
        self.status_label = ctk.CTkLabel(top, text="Chua tai map", font=("Consolas", 11, "bold"), text_color=C_SUBTEXT)
        ...
        ctk.CTkLabel(side, text="TRAM TREN MAP", font=("Consolas", 15, "bold"), text_color=C_ACCENT).pack(anchor="w", padx=12, pady=(12, 6))
        ...
        ctk.CTkButton(form, text="ADD", width=60, command=self.add_station).pack(side="left", padx=4, pady=10)

        ctk.CTkButton(side, text="LAY TOA DO ROBOT -> TRAM", command=self.add_station_from_robot).pack(fill="x", padx=10, pady=4)
        ctk.CTkButton(side, text="XOA TRAM CUOI", fg_color=C_ORANGE, command=self.remove_last_station).pack(fill="x", padx=10, pady=4)
        self.station_box = ctk.CTkTextbox(side, height=130, font=("Consolas", 10))
        self.station_box.pack(fill="x", padx=10, pady=(8, 12))

        ctk.CTkLabel(side, text="CALIB CAMERA ↔ SLAM MAP", font=("Consolas", 15, "bold"), text_color=C_ACCENT).pack(anchor="w", padx=12, pady=(8, 6))
        calib = ctk.CTkFrame(side, fg_color=C_CARD, corner_radius=8)
        calib.pack(fill="x", padx=10, pady=8)
        ctk.CTkLabel(calib, text="1) Click tren camera live o tab CONTROL XE (se hien cham do P1,P2...)\n2) Double-click/chuot phai tren map dung diem tuong ung (se hien cham do)\nCan toi thieu 4 cap diem, thu tu CAM Pn phai khop MAP Pn.", justify="left", font=("Consolas", 10), text_color=C_SUBTEXT).pack(anchor="w", padx=10, pady=(10, 6))
        rowc = ctk.CTkFrame(calib, fg_color="transparent"); rowc.pack(fill="x", padx=10, pady=3)
        self.cam_u = ctk.CTkEntry(rowc, width=70, placeholder_text="u px"); self.cam_u.pack(side="left", padx=3)
        self.cam_v = ctk.CTkEntry(rowc, width=70, placeholder_text="v px"); self.cam_v.pack(side="left", padx=3)
        ctk.CTkButton(rowc, text="ADD CAM", width=85, command=self.add_camera_point_manual).pack(side="left", padx=3)
        rowm = ctk.CTkFrame(calib, fg_color="transparent"); rowm.pack(fill="x", padx=10, pady=3)
        self.map_x = ctk.CTkEntry(rowm, width=70, placeholder_text="x m"); self.map_x.pack(side="left", padx=3)
        self.map_y = ctk.CTkEntry(rowm, width=70, placeholder_text="y m"); self.map_y.pack(side="left", padx=3)
        ctk.CTkButton(rowm, text="ADD MAP", width=85, command=self.add_map_point_manual).pack(side="left", padx=3)
        rowd = ctk.CTkFrame(calib, fg_color="transparent"); rowd.pack(fill="x", padx=10, pady=3)
        self.del_p = ctk.CTkEntry(rowd, width=70, placeholder_text="so P")
        self.del_p.pack(side="left", padx=3)
        ctk.CTkButton(rowd, text="XOA CAM P", width=90, fg_color=C_RED, command=self.delete_camera_point_by_entry).pack(side="left", padx=3)
        ctk.CTkButton(rowd, text="XOA MAP P", width=90, fg_color=C_RED, command=self.delete_map_point_by_entry).pack(side="left", padx=3)
        ctk.CTkButton(rowd, text="XOA CAP P", width=90, fg_color=C_ORANGE, command=self.delete_pair_by_entry).pack(side="left", padx=3)
        ctk.CTkButton(calib, text="TINH HOMOGRAPHY", fg_color=C_GREEN, command=self.compute_homography).pack(fill="x", padx=10, pady=(8, 3))
        ctk.CTkButton(calib, text="XOA DIEM CALIB", fg_color=C_ORANGE, command=self.clear_calibration_points).pack(fill="x", padx=10, pady=3)
        ...
        ctk.CTkLabel(side, text="ROBOT / ROS2", font=("Consolas", 15, "bold"), text_color=C_ACCENT).pack(anchor="w", padx=12, pady=(8, 6))
        ...
        ctk.CTkButton(pose_frame, text="SET ROBOT", command=self.set_robot_manual).grid(row=1, column=0, columnspan=3, sticky="ew", padx=6, pady=(0,8))
    ...
    def _on_map_double_click(self, event):
        ...
        self._refresh_calib_box()
        self._log(f"ADD MAP point P{len(self.calib_map_points)}: x={pt[0]:.3f}, y={pt[1]:.3f}")
    ...
    def add_camera_point_manual(self):
        try:
            u = float(self.cam_u.get().strip()); v = float(self.cam_v.get().strip())
            self.add_camera_point_from_click(u, v)
            self.cam_u.delete(0, "end"); self.cam_v.delete(0, "end")
        except Exception as e:
            self._log(f"LOI ADD CAM point: {e}")

    def add_map_point_manual(self):
        try:
            x = float(self.map_x.get().strip()); y = float(self.map_y.get().strip())
            self.calib_map_points.append((x, y))
            self._refresh_calib_box()
            self.map_x.delete(0, "end"); self.map_y.delete(0, "end")
    ...
    def delete_pair_point(self, idx):
        ok = False
        # Xoa MAP truoc/CAM sau cung index de giu thu tu cac P con lai.
        if idx is not None and 0 <= idx < len(self.calib_map_points):
            self.calib_map_points.pop(idx); ok = True
        if idx is not None and 0 <= idx < len(self.calib_camera_points):
            self.calib_camera_points.pop(idx); ok = True
        if ok:
            self.shared["camera_calib_points"] = list(self.calib_camera_points)
            self._refresh_calib_box()
    ...
    def clear_calibration_points(self):
        self.calib_camera_points.clear(); self.calib_map_points.clear()
        self._refresh_calib_box()
    ...
    def _refresh_calib_box(self):
        ...
        self.calib_box.insert("end", f"CAM points: {len(self.calib_camera_points)} | MAP points: {len(self.calib_map_points)}\n")
        for i in range(n):
        ...
        self.calib_box.insert("end", "\nChon xoa: nhap 3/P3 hoac chuot phai gan cham tren camera/map.\n")
        self.calib_box.insert("end", "Trang thai H: " + ("DA CO" if self.homography is not None else "CHUA CO") + "\n")
    ...
    def save_homography_dialog(self):
        path = filedialog.asksaveasfilename(title="Luu homography.yaml", defaultextension=".yaml", filetypes=[("YAML", "*.yaml *.yml"), ("All files", "*.*")])
        if path:
    ...
    def load_homography_dialog(self):
        path = filedialog.askopenfilename(title="Chon homography.yaml", filetypes=[("YAML", "*.yaml *.yml"), ("All files", "*.*")])
        if path:
    ...
    def load_yaml_dialog(self):
        path = filedialog.askopenfilename(title="Chon map.yaml", filetypes=[("YAML map", "*.yaml *.yml"), ("All files", "*.*")])
        if path:
    ...
    def load_map_yaml(self, path):
        try:
            ...
            # ROS map pgm: den = occupied, trang = free. Invert nhe de de nhin tren nen toi.
            ...
            self.status_label.configure(text=f"Map: {img.width}x{img.height}, res={self.resolution}", text_color=C_GREEN)
    ...
    def fit_map(self):
        ...
        self.zoom = min((cw - 30) / self.map_image.width, (ch - 30) / self.map_image.height)
    ...
    def redraw(self):
        ...
        if not self.map_image:
            self.canvas.create_text(30, 30, anchor="nw", text="Chua tai map.yaml", fill="#94a3b8", font=("Consolas", 16))
            return
        ...
        self.canvas.create_rectangle(self.offset_x, self.offset_y, self.offset_x + w, self.offset_y + h, outline="#38bdf8")
        ...
        # calibration map points - red dots selected by double-click / right-click
        for i, (x, y) in enumerate(self.calib_map_points, start=1):
            cx, cy = self.map_to_canvas(x, y)
            r = 7
            self.canvas.create_oval(cx-r, cy-r, cx+r, cy+r, fill="#ef4444", outline="#ffffff", width=2)
            self.canvas.create_text(cx, cy, text=str(i), fill="#ffffff", font=("Consolas", 9, "bold"))
            self.canvas.create_text(cx + 11, cy + 8, text=f"P{i} ({x:.2f},{y:.2f})", anchor="nw", fill="#fecaca", font=("Consolas", 9, "bold"))

        # stations
        for name, (x, y) in sorted(self.stations.items()):
            cx, cy = self.map_to_canvas(x, y)
            r = 8
            self.canvas.create_oval(cx-r, cy-r, cx+r, cy+r, fill="#f59e0b", outline="#ffffff", width=2)
            self.canvas.create_text(cx + 12, cy - 12, text=f"TRAM {name}\n{x:.2f},{y:.2f}", anchor="w", fill="#fde68a", font=("Consolas", 10, "bold"))
        ...
        if self.robot_pose:
            ...
            cx, cy = self.map_to_canvas(x, y)
            r = 10
            self.canvas.create_oval(cx-r, cy-r, cx+r, cy+r, fill="#00d4ff", outline="#ffffff", width=2)
            # yaw arrow, ROS yaw x-axis
            ex = cx + math.cos(yaw) * 28
            ey = cy - math.sin(yaw) * 28
            self.canvas.create_line(cx, cy, ex, ey, fill="#ffffff", width=3, arrow=tk.LAST)
            self.canvas.create_text(cx + 14, cy + 12, text=f"ROBOT\nx={x:.2f} y={y:.2f}\nyaw={math.degrees(yaw):.1f}", anchor="nw", fill="#bae6fd", font=("Consolas", 10, "bold"))

        self.canvas.create_text(10, 10, anchor="nw", text=f"zoom={self.zoom:.2f} | keo chuot de pan | cuon chuot de zoom", fill="#e2e8f0", font=("Consolas", 10))
